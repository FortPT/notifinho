"""Dependency-free privacy helpers shared by discovery utilities."""
...
_SECRET_ASSIGNMENT_RE = re.compile(
    r"(?i)\b(authorization|api[_-]?key|access[_-]?token|refresh[_-]?token|token|cookie|set-cookie|secret|password|webhook(?:_url)?)"
    r"(\s*[:=]\s*)([^\s,;]+)"
)
...
_SERIAL_RE = re.compile(
    r"(?i)\b(serial(?:[_ -]?number)?|device[_ -]?id|disk[_ -]?id|endpoint[_ -]?id|environment[_ -]?id)"
