"""Temporary standard-library HTTP capture server for Portainer discovery."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a temporary sanitized Portainer webhook capture server.",
    )
    parser.add_argument("--host", default="127.0.0.1", help="bind host")
    parser.add_argument("--port", type=int, default=18083, help="bind port")
    parser.add_argument(
        "--allow-get",
        action="store_true",
...
def safe_raw_filename(sequence: int, method: str) -> str:
    ...
    safe_method = (
        candidate
        if candidate in {"delete", "get", "patch", "post", "put"}
        else "request"
    )
